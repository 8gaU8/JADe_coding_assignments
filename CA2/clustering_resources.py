...
class RandomNumGen:
    ...
    def set_seed(tcl, seed=None):
        tcl.seed = seed
        if seed is not None:
            tcl.generator = numpy.random.Generator(
                numpy.random.MT19937(numpy.random.SeedSequence(seed))
            )
        else:
            tcl.generator = None
...
def normalize_dataset(dataset, minmax=None):
    ...
    return (dataset - numpy.tile(minmax[0, :], (dataset.shape[0], 1))) / numpy.tile(
        minmax[1, :] - minmax[0, :], (dataset.shape[0], 1)
    )
...
def sample_domain(k, minmax=None, dataset=None):
    ...
    return numpy.tile(minmax[0, :], (k, 1)) + d * numpy.tile(
        minmax[1, :] - minmax[0, :], (k, 1)
    )
...
def L2_distance(A, B):
    return numpy.vstack(
        [
            numpy.sqrt(
                numpy.sum((A - numpy.tile(B[i, :], (A.shape[0], 1))) ** 2, axis=1)
            )
...
def L1_distance(A, B):
    return numpy.vstack(
        [
            numpy.sum(numpy.abs(A - numpy.tile(B[i, :], (A.shape[0], 1))), axis=1)
            for i in range(B.shape[0])
        ]
    ).T
...
def intra_inter_distance_ratio(distM, clabels):
    ...
    mask_sameclass = (
        (Ct == Ct.T) & (Ct != -1) & (numpy.diag(numpy.ones(len(clabels))) != 1)
    )
...
def clust_silhouette(distM, clabels):
    ...
    countsM = numpy.tile(
        numpy.array([numpy.sum(clabels == l) for l in uniq_labels]), (len(clabels), 1)
    ).T
    ## no discount for singleton clusters to avoid division by zero
    discountM = numpy.array(
        [1 * ((clabels == l) & (numpy.sum(clabels == l) > 1)) for l in uniq_labels]
    )
...
def purity_CM(contingency_matrix):
    return numpy.sum(numpy.max(contingency_matrix, axis=0)) / numpy.sum(
        contingency_matrix
    )
    ...
    return numpy.sum(
        (
            ...
            - numpy.sum(
                numpy.divide(
                    contingency_matrix,
                    numpy.tile(marg_rows, (contingency_matrix.shape[0], 1)),
                )
                ** 2,
                axis=0,
            )
...
def entropy_CM(contingency_matrix):
    marg_rows = numpy.sum(contingency_matrix, axis=0)
    ps = numpy.divide(
        contingency_matrix, numpy.tile(marg_rows, (contingency_matrix.shape[0], 1))
    )
    ps[contingency_matrix == 0] = 1
    return numpy.sum(
        numpy.sum(-ps * numpy.log2(ps), axis=0)
        * marg_rows
        / numpy.sum(contingency_matrix)
    )
...
if __name__ == "__main__":
    ...
    clabelsA = numpy.floor(
        5 * (numpy.arange(dataset.shape[0])[::-1] / dataset.shape[0])
    )
    clabelsB = numpy.floor(
        5 * ((numpy.arange(dataset.shape[0]) / dataset.shape[0]) ** 2)
    )

    ### matrix of pairwise distances between points in the dataset
    distM = L2_distance(Dn, Dn)
    print(
        "Clustering A:\t SSDC=%.4f, DR=%.4f, S=%.4f"
        % (
            ss_distances_centroids(Dn, clabelsA),
    ...
    print(
        "contingency matrix:\n", cm
    )  ## what do the rows and the columns respectively represent?
